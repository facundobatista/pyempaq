...
stowed_filepath = pathlib.Path.cwd() / sys.argv[0]
zf = zipfile.ZipFile(stowed_filepath)
metadata = json.loads(zf.read("metadata.json").decode("utf8"))
log("Loaded metadata: {}", metadata)

# load appdirs from the builtin venv
sys.path.insert(0, f"{stowed_filepath}/venv/")
...
timestamp = time.strftime("%Y%m%d%H%M%S", time.gmtime(stowed_filepath.stat().st_ctime))
